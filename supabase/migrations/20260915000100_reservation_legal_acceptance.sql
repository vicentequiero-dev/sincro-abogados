...
alter table public.reservations
  ...
  add constraint reservations_legal_acceptance_shape
    check (
      (
...
drop function public.create_reservation_hold_limited(
  text,
  text,
  text,
  text,
...
create function public.create_reservation_hold_limited(
  p_ip_hash text,
...
begin
  ...
  if p_ip_hash is null
     ...
     or p_legal_accepted is distinct from true
     or p_terms_version is distinct from '1.0'
     or p_privacy_version is distinct from '1.0'
  then
  ...
  -- Preserve the existing deterministic lock order: IP, email, phone.
